...
@Component({
  ...
  styleUrls: ['./detail-delivery-order.component.css']
})
export class DetailDeliveryOrderComponent implements OnInit {
  ...
  showDetail() {
    ...
    let url = "do/" + this.apiBackend.serviceAuth()['tenant_id'] + '/finish/' + idDetail
    this.apiBackend.GetSearch(url).subscribe((data: {}) => {
      ...
      this.date = this.dataDetail.data.order.delivery_date
