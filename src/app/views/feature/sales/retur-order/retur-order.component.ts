...
@Component({
  selector: 'app-retur-order',
  templateUrl: './retur-order.component.html',
  styleUrls: ['./retur-order.component.css']
})
export class ReturOrderComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  urlCreate = this.router.url + "/create-retur"
  ...
  ngOnInit() {
    ...
    fromEvent(this.searchInput.nativeElement, 'keyup').pipe(
      ...
      if (text == null || text == "") {
        ...
        let url = 'return/' + this.apiBackend.serviceAuth()['tenant_id'] + '/search/' + text
  ...
  table = false
  getAll() {
    this.apiBackend.Get('return').subscribe((data: {}) => {
      this.dataApi = data
  ...
  formUpdate(id) {
    let urlUpdate = this.router.url + '/update-retur/' + id
    this.router.navigate([urlUpdate]);
  }
  formDetail(id) {
    let urlUpdate = this.router.url + '/detail-retur/' + id
    this.router.navigate([urlUpdate]);
  }
  onDelete() {
