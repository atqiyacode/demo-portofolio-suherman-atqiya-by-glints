...
export class UpdateDiscountComponent implements OnInit, AfterContentChecked {
  constructor(
  ...
  getDetail(){
    ...
    this.apiBackend.Show(url).subscribe(
      (data: {}) => {
        ...
        this.detailStartDate = moment(start).format('Y-MM-Do H:mm:ss');
        this.detailEndDate = moment(end).format('Y-MM-Do H:mm:ss');
  ...
  checkAddProduct(id){
    const url = `discount/${this.apiBackend.serviceAuth().tenant_id}/check-product/${this.idDetail}/${id}?startdate=${this.detailStartDate}&enddate=${this.detailEndDate}`;
